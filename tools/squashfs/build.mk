...
$(WORK_DIR)/$(ZLIB_NAME)/.configured: $(WORK_DIR)/.copied
	cd $(dir $@) && ./configure --prefix=../prefix --static
	@touch $@
...
$(PREFIX_DIR)/lib/$(LZ4_TARGET): $(WORK_DIR)/$(LZ4_NAME)/lib/$(LZ4_TARGET)
	+$(MAKE) -C $(dir $<) BUILD_SHARED=no PREFIX=../../prefix install

$(PREFIX_DIR)/lib/$(ZSTD_TARGET): $(WORK_DIR)/.copied
	+$(MAKE) -C $(WORK_DIR)/$(ZSTD_NAME)/lib PREFIX=../../prefix install-static install-includes
